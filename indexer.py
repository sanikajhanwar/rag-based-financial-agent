        ...
        for i, form in enumerate(filings['form']):
            if form == '10-K':
                ...
                if dl_response.status_code == 200:
                    ...
                    # --- OPTIMIZED BATCHING WITH RETRY ---
                    BATCH_SIZE = 15  # Faster batch size
                    total_batches = (len(chunks) + BATCH_SIZE - 1) // BATCH_SIZE
                    
                    yield json.dumps({"type": "log", "message": f"   -> Indexing {len(chunks)} chunks..."}) + "\n"

                    for k in range(0, len(chunks), BATCH_SIZE):
                        batch_chunks = chunks[k : k + BATCH_SIZE]
                        batch_ids = ids[k : k + BATCH_SIZE]
                        batch_meta = metadatas[k : k + BATCH_SIZE]
                        
                        # RETRY LOGIC: Try 3 times if API fails
                        max_retries = 3
                        for attempt in range(max_retries):
                            try:
                                ...
                                if attempt == max_retries - 1:
                                    yield json.dumps({"type": "error", "message": f"Batch failed: {str(e)}"}) + "\n"
                                else:
                                    wait_time = (attempt + 1) * 2
                                    yield json.dumps({"type": "log", "message": f"⚠️ API busy, retrying in {wait_time}s..."}) + "\n"
                                    time.sleep(wait_time)
                        
                        # Log progress occasionally
                        if k % (BATCH_SIZE * 2) == 0:
                             yield json.dumps({"type": "log", "message": f"   -> Progress: {int((k/len(chunks))*100)}%"}) + "\n"
